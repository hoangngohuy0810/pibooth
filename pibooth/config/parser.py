...
DEFAULT = odict((
    ...
    ("WINDOW",
        odict((
            ...
            ("review_picture",
                (True,
                 "Keep the final picture on screen until the user explicitly continues",
                 "Review picture after capture", ['True', 'False'])),
            ("review_picture_text",
                ("Ảnh vừa chụp  •  Chạm màn hình hoặc bấm Space để tiếp tục",
                 "Text displayed on the final picture review screen",
                 None, None)),
            ("wait_picture_delay",
                (-1,
